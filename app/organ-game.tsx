...
import type { AugmentTier, CardKind, Choice, CoreOrgan, Difficulty, Game, MainClass, Mob, Mode, OrganKey, RunTelemetry, SkillFx } from "./game/types";
...
const LIFE: Choice[][] = [
  [
    ...
    {name:"매점 풀코스",desc:"점심시간보다 매점 시간이 더 기다려졌습니다.",effect:"즉시 회복 · 공격 속도 증가 · 간 약화",apply:g=>{healPlayer(g,28);g.organs.심장+=5;g.organs.간-=8;g.fireRate*=.9}},
  ],
  [
  ...
  [
    {name:"배달 야식",desc:"당장의 위기를 넘기는 대신 회복 효율을 희생합니다.",effect:"즉시 체력 +45 · 피해 +8",cost:"간·폐 감소 · 이후 회복량 -20%",apply:g=>{healPlayer(g,45);g.damage+=8;g.organs.간-=10;g.organs.폐-=5;g.recoveryPenalty=Math.min(.7,g.recoveryPenalty+.2)}},
  ...
  [
    ...
    {name:"명상과 산책",desc:"빠르게 가는 대신 오래 가는 법을 배웠습니다.",effect:"뇌·폐 회복 · 피해 감소",apply:g=>{g.organs.뇌+=10;g.organs.폐+=10;g.maxHp+=18;healPlayer(g,18)}},
  ],
];
const AUG: Choice[] = [
  ...
  {name:"심폐 순환",desc:"계속 움직일수록 다시 살아납니다.",effect:"최대 체력 +22 · 속도 증가",apply:g=>{g.maxHp+=22;healPlayer(g,22);g.speed+=12}},
...
const BASIC: Choice[] = [
  ...
  {name:"심실 강화",desc:"더 큰 충격을 견딜 수 있습니다.",effect:"최대 체력 +15 · 즉시 회복",apply:g=>{g.maxHp+=15;healPlayer(g,15)}},
  {name:"재생 인자",desc:"손상된 조직이 빠르게 회복됩니다.",effect:"체력 35 회복",apply:g=>{healPlayer(g,35)}},
  ...
  {name:"세포막 경화",desc:"외부 충격을 버티는 막이 두꺼워집니다.",effect:"방어력 +3 · 최대 체력 +8",apply:g=>{g.armor+=3;g.maxHp+=8;healPlayer(g,8)}},
];
...
const gameRandom=(g:Game)=>{let t=g.randomState=(g.randomState+0x6d2b79f5)>>>0;t=Math.imul(t^(t>>>15),t|1);t^=t+Math.imul(t^(t>>>7),t|61);return((t^(t>>>14))>>>0)/4294967296};
const shuffled=<T,>(g:Game,items:T[])=>{const copy=[...items];for(let i=copy.length-1;i>0;i--){const j=Math.floor(gameRandom(g)*(i+1));[copy[i],copy[j]]=[copy[j],copy[i]]}return copy};
...
const pickTieredClassCard=(g:Game,excluded:Set<string>,weakestTier:AugmentTier=4):Choice|undefined=>{
  const card=pickTieredAugment({state:g,cards:CLASS_CARDS.filter((candidate):candidate is CardDef&{main:CoreOrgan;tier:AugmentTier}=>candidate.main!==undefined&&candidate.tier!==undefined),catalog:AUGMENT_CATALOG,config:AUGMENT_BALANCE,excluded,weakestTier,random:()=>gameRandom(g)});
...
const weightedChoices=(g:Game,source:ChoiceSource="level"):Choice[]=>{
  ...
  while(picks.length<3){
    ...
    let roll=gameRandom(g)*usable.reduce((s,p)=>s+p.w,0),pool=usable[0];for(const p of usable){roll-=p.w;if(roll<=0){pool=p;break}}
    const raw=pool.kind==="class"?pickTieredClassCard(g,used):shuffled(g,pool.c??[]).find(choice=>!used.has(choiceKey(choice)));if(!raw)continue;picks.push(raw);
  }
...
export default function OrganGame() {
  ...
  const [report,setReport]=useState({win:false,kills:0,t:0,organs:game.current.organs,choices:[] as string[],augments:[] as string[],mainClass:null as MainClass,fusions:[] as string[],telemetry:null as RunTelemetry|null});
  ...
  const endGame=useCallback((win:boolean)=>{
    ...
    const telemetry=finalizeTelemetry(g,win),history=[...readTelemetryHistory(),telemetry].slice(-100);localStorage.setItem(TELEMETRY_STORAGE_KEY,JSON.stringify(history));setTelemetryRuns(history);
    if(g.benchmark&&g.benchmarkTarget)setBenchmarkBatch(old=>{const next=[...old,telemetry];if(benchmarkBatchKey.current)localStorage.setItem(benchmarkBatchKey.current,JSON.stringify(next));return next});
    setReport({win,kills:g.kills,t:g.t,organs,choices:[...g.choices],augments:[...g.augments],mainClass:g.mainClass,fusions,telemetry});
    const strongest=ORGAN_KEYS.reduce((a,b)=>organs[a]>organs[b]?a:b);
    if(!g.debug&&!g.benchmark){localStorage.setItem("organ-gene",strongest);const bestKills=Math.max(g.kills,Number(localStorage.getItem("organ-best-kills")||0)),bestTime=Math.max(g.t,Number(localStorage.getItem("organ-best-time")||0));localStorage.setItem("organ-best-kills",String(bestKills));localStorage.setItem("organ-best-time",String(bestTime));setArchive(old=>({...old,gene:strongest,bestKills,bestTime}))}
  ...
  const start=useCallback((difficulty:Difficulty="normal")=>{
    const params=new URLSearchParams(window.location.search),preview=typeof window!=="undefined"&&params.get("preview")!==null;
    ...
    const g=fresh(difficulty),benchmarkTarget=params.get("autoplay") as CoreOrgan|null;g.benchmark=params.get("benchmark")==="1";g.benchmarkTarget=g.benchmark&&(["heart","brain","liver","lung","muscle"] as CoreOrgan[]).includes(benchmarkTarget)?benchmarkTarget:null;g.benchmarkSpeed=g.benchmark?Math.max(1,Math.min(60,Math.floor(Number(params.get("speed"))||1))):1;g.benchmarkStopAt=g.benchmark?Math.max(0,Math.min(RUN_TARGET,Number(params.get("duration"))||0)):0;const gene=localStorage.getItem("organ-gene") as OrganKey|null;
    if(g.benchmarkTarget&&runNumber.current===0){benchmarkBatchKey.current=`organ-benchmark-${params.get("batch")||"default"}-${g.benchmarkTarget}`;const saved=JSON.parse(localStorage.getItem(benchmarkBatchKey.current)||"[]") as RunTelemetry[];benchmarkRunOffset.current=saved.length;setBenchmarkBatch(saved)}
    const requestedSeed=Number(params.get("seed")),baseSeed=Number.isFinite(requestedSeed)&&params.has("seed")?Math.trunc(requestedSeed)>>>0:(Date.now()^Math.floor(Math.random()*4294967296))>>>0;g.randomSeed=(baseSeed+benchmarkRunOffset.current+runNumber.current)>>>0||1;g.randomState=g.randomSeed;
  ...
  useEffect(()=>{
    if(mode!=="report"||!report.telemetry?.benchmark||!report.telemetry.benchmarkTarget||benchmarkBatch.length>=benchmarkRunTarget)return;
  ...
  const dashNow=useCallback(()=>{
    ...
    const active=g.mainClass;g.dashCharges--;g.telemetry.actionsUsed++;if(g.dash<=0)g.dash=1.55;
    if(active==="heart"){
      g.vx=dx*690;g.vy=dy*690;g.inv=.3;g.dashFx=.34;g.heartFx=.42;g.shake=13;
      for(const m of g.mobs){const tx=m.x-g.x,ty=m.y-g.y,d=Math.hypot(tx,ty),facing=(tx*dx+ty*dy)/Math.max(1,d);if(d<155&&facing>.25){dealDamage(g,m,g.damage*2.1,"heart_action");m.hit=.12}}
      ...
      targets.forEach((m,i)=>{const a=Math.atan2(m.y-g.y,m.x-g.x)+(i%2?-.035:.035);g.shots.push({x:g.x,y:g.y,vx:Math.cos(a)*650,vy:Math.sin(a)*650,life:1.4,r:7,source:"brain_action"})});
      ...
      for(const m of g.mobs){const tx=m.x-g.x,ty=m.y-g.y,d=Math.hypot(tx,ty),facing=(tx*dx+ty*dy)/Math.max(1,d);if(d<210&&facing>.15){dealDamage(g,m,g.damage*1.8,"lung_action");m.hit=.12;const gd=Math.hypot(m.x-gx,m.y-gy)||1;m.kbX+=(m.x-gx)/gd*230;m.kbY+=(m.y-gy)/gd*230}}
      ...
      for(const m of g.mobs){const d=Math.hypot(m.x-g.x,m.y-g.y);if(d<radius+m.r){dealDamage(g,m,dmg,"muscle_action");m.hit=.14;const nx=(m.x-g.x)/(d||1),ny=(m.y-g.y)/(d||1);m.kbX+=nx*kb;m.kbY+=ny*kb}}
  ...
  useEffect(()=>{
    ...
    const spawn=(g:Game,boss=false)=>{
      const angle=gameRandom(g)*Math.PI*2,distance=Math.max(g.w,g.h)*(.62+gameRandom(g)*.2);
      ...
      const km=kind===1?{r:1.28,hp:1.4,spd:.82}:kind===2?{r:.78,hp:.64,spd:1.34}:{r:1,hp:1,spd:1};
      const mr=(boss?(g.stage===3?52:38):(10+gameRandom(g)*8)*km.r)*(coarse ? .8 : 1);
      const mhp=boss?base*18:base*km.hp;
      const mspd=(boss?58:(65+gameRandom(g)*44+g.stage*8)*km.spd)*diff.speed;
      g.mobs.push({x,y,r:mr,hp:mhp,max:mhp,speed:mspd,boss,elite:boss||gameRandom(g)<.08+g.stage*.025,kind,hit:0,skill:1.5+gameRandom(g)*3,cast:0,charge:0,aimX:x,aimY:y,toxin:0,poisonStacks:0,poisonTick:0,overloadHits:0,heartMark:0,kbX:0,kbY:0,collideCd:0});
    };
    ...
    const simulateStep=(g:Game,dt:number)=>{
        g.t+=dt;g.stageT+=dt;g.dash-=dt;g.inv-=dt;g.fire-=dt;g.effectT-=dt;g.dashFx-=dt;g.castFx-=dt;g.heartFx-=dt;g.moveBuff-=dt;g.toxicCoreCooldown-=dt;g.noDamage+=dt;g.shake=Math.max(0,g.shake-dt*30);g.hurtT=Math.max(0,g.hurtT-dt*2);
        g.benchmarkActionCooldown-=dt;if(g.benchmarkTarget&&g.mainClass&&g.dashCharges>0&&g.benchmarkActionCooldown<=0){g.benchmarkActionCooldown=.7;dashNow()}
        ...
        if(!g.choiceDone&&g.stageT>(g.stage===0?FIRST_CHOICE_AT:LATER_CHOICE_AT)){g.choiceDone=true;const life=available(g,LIFE_CARDS);let picks:Choice[];if(!g.awakened){
          // 미각성 상태의 생활 선택에는 장기 성장 카드 1장을 보장한다: "무엇을 키워야 각성하는지" 방향을 준다
          const organCard=shuffled(g,ORGAN_GROWTH.filter(d=>d.organLevel&&g.organLevels[d.organLevel]<3))[0];
          const others=shuffled(g,life).slice(0,organCard?2:3).map(toChoice);
          picks=organCard?shuffled(g,[organCard,...others]):(others.length?others:weightedChoices(g));
        }else{picks=life.length?shuffled(g,life).slice(0,3).map(toChoice):weightedChoices(g);}
          openChoice("생활 선택",picks);}
        ...
        const moveSpeed=g.speed*moveBuffMul*(g.mainClass==="lung"?1+g.galeMomentum*.08:1);
        if(g.inv<=.12){const power=Math.min(1,n);g.vx=dx/n*moveSpeed*power;g.vy=dy/n*moveSpeed*power}
        const previousX=g.x,previousY=g.y;g.x=Math.max(18,Math.min(g.worldW-18,g.x+g.vx*dt));g.y=Math.max(18,Math.min(g.worldH-18,g.y+g.vy*dt));g.telemetry.distanceTraveled+=Math.hypot(g.x-previousX,g.y-previousY);
        if(g.mainClass==="liver"&&moving){
          ...
          if(g.poisonTrailDistance>=interval){g.poisonTrailDistance=0;const overlapLevel=cardLevel(g,"liver_overlap"),overlap=overlapLevel?g.fields.find(f=>Math.hypot(f.x-g.x,f.y-g.y)<f.r*.58):undefined;if(overlap){const maxStack=levelValue(AUGMENT_BALANCE.liverOverlap.maxStacks,overlapLevel);if(overlap.stack<maxStack){overlap.stack++;overlap.r*=AUGMENT_BALANCE.liverOverlap.radiusGrowth}overlap.life=Math.max(overlap.life,4.8*g.poisonDurationMul);g.effect=`오염 중첩 · ${overlap.stack}/${maxStack}단계`;g.effectT=.7}else g.fields.push({x:g.x,y:g.y,r:AUGMENT_BALANCE.liverOverlap.baseRadius*g.poisonRadiusMul,life:(4.8+(cardLevel(g,"liver_footprints")*.9))*g.poisonDurationMul,stack:1,kills:0,tick:0});g.fields=g.fields.slice(-36);pushSkill(g,"liver",0,g.x,g.y,52,.45,{rot:Math.atan2(g.vy,g.vx),grow:1.1})}
        ...
        if(g.mainClass==="lung"){
          ...
          if(bw&&moving){g.windTrailDist+=Math.hypot(g.vx,g.vy)*dt;const iv=95*Math.pow(.78,bw-1);if(g.windTrailDist>=iv){g.windTrailDist=0;const a=Math.atan2(g.vy,g.vx);g.shots.push({x:g.x,y:g.y,vx:Math.cos(a)*710,vy:Math.sin(a)*710,life:.6,r:13,damageMul:1.1+g.galeMomentum*.15,source:"lung_bladewind"});pushSkill(g,"lung",0,g.x,g.y,60,.3,{rot:a,grow:1.4})}}
          if(cardLevel(g,"lung_afterimage")&&moving&&g.galeMomentum>=3.2){for(const m of g.mobs)if(Math.hypot(m.x-g.x,m.y-g.y)<72)dealDamage(g,m,(6+cardLevel(g,"lung_afterimage")*3)*dt,"lung_afterimage");if(Math.random()<dt*22)pushSkill(g,"lung",2,g.x-g.vx*.03,g.y-g.vy*.03,54,.3,{rot:Math.atan2(g.vy,g.vx),grow:1.1})}
          if(cardLevel(g,"lung_eyestorm")&&moving&&Math.floor(g.t*2)!==Math.floor((g.t-dt)*2)){const cnt=cardLevel(g,"lung_eyestorm");for(let i=0;i<cnt;i++){const a=gameRandom(g)*6.28,r=58+gameRandom(g)*44,wx=g.x+Math.cos(a)*r,wy=g.y+Math.sin(a)*r;for(const m of g.mobs)if(Math.hypot(m.x-wx,m.y-wy)<52)dealDamage(g,m,g.damage*.5,"lung_eyestorm");pushSkill(g,"lung",5,wx,wy,72,.4,{spin:6,grow:1.3})}}
        }
        ...
        const hurtPlayer=(raw:number,srcX?:number,srcY?:number)=>{if(g.invuln)return false;let amount=takeDamage(raw);g.noDamage=0;g.hurtT=.55;if(srcX!==undefined&&srcY!==undefined)g.hurtDir=Math.atan2(srcY-g.y,srcX-g.x);g.telemetry.hitsTaken++;if(g.mainClass==="muscle"){const pf=cardLevel(g,"muscle_painfuel");if(pf)g.impactCharge=Math.min(1,g.impactCharge+.05*pf)}if(g.shield>0){const blocked=Math.min(g.shield,amount);g.shield-=blocked;amount-=blocked;g.telemetry.damageBlocked+=blocked}const hpLoss=Math.min(g.hp,amount);g.hp-=amount;g.telemetry.damageTaken+=Math.max(0,hpLoss);if(g.hp<=0&&g.reviveAvailable){g.reviveAvailable=false;const revived=g.maxHp*.4;g.hp=0;healPlayer(g,revived);g.inv=2;g.shield=g.maxHp*.15;g.heartFx=.8;g.effect="세포 분열 · 40% 체력으로 부활";g.effectT=3;sound.current?.play("level");return false}if(g.hp<=0){endGame(false);return true}
          // 저체력 진입 시 1회 경고음+문구, 42% 이상 회복하면 재무장
          if(g.hp/g.maxHp<.3&&!g.lowHpWarned){g.lowHpWarned=true;g.effect="위험 · 체력이 낮습니다";g.effectT=1.6;sound.current?.play("hurt")}else if(g.hp/g.maxHp>=.42)g.lowHpWarned=false;
          return false};
        ...
        if(g.mobs.filter(m=>!m.boss).length<cap&&gameRandom(g)<dt*(5+g.stage*3+g.stageT*.05)*diff.count)spawn(g);
        let nearest:Mob|undefined,nd=Infinity;for(const m of g.mobs){const d=(m.x-g.x)**2+(m.y-g.y)**2;if(d<nd){nd=d;nearest=m}}
        if(g.fire<=0&&nearest){
          const shootAt=(target:Mob,opts:{damageMul?:number;chain?:number;poison?:number;core?:boolean;angle?:number;source?:string}={})=>{const a=opts.angle??Math.atan2(target.y-g.y,target.x-g.x);g.shots.push({x:g.x,y:g.y,vx:Math.cos(a)*580,vy:Math.sin(a)*580,life:1.6,r:opts.core?7:5,damageMul:opts.damageMul??1,chain:opts.chain??0,poison:opts.poison??0,core:opts.core,source:opts.source??"basic_projectile"})};
          g.shotCount++;
          if(g.mainClass==="heart"){
            ...
            if(inRange){g.meleeCombo=(g.meleeCombo%4)+1;inRange.overloadHits++;let damage=g.damage*1.18*(1+cardLevel(g,"heart_overload")*.08),source="heart_basic";if(cardLevel(g,"heart_overload")&&inRange.overloadHits>=5){damage*=2.2;source="heart_overload";inRange.overloadHits=0;pushSkill(g,"heart",4,inRange.x,inRange.y,120,.4,{grow:1.6});g.effect="과부하 연타 · 2.2×";g.effectT=.8}if(inRange.heartMark>0){damage*=1.3;inRange.heartMark=0;pushSkill(g,"heart",6,inRange.x,inRange.y,64,.35,{grow:1.3})}dealDamage(g,inRange,damage,source);inRange.hit=.12;g.heartFx=.2;sound.current?.play("hit");burst(g,inRange.x,inRange.y,"#ff715b",6);if(g.meleeCombo<4)pushSkill(g,"heart",g.meleeCombo-1,inRange.x,inRange.y,72,.2,{rot:Math.atan2(inRange.y-g.y,inRange.x-g.x),grow:1.2});
              if(cardLevel(g,"fusion_heart_liver")){inRange.poisonStacks=Math.min(8,inRange.poisonStacks+1);inRange.toxin=4}
              if(g.meleeCombo===4){const shock=cardLevel(g,"heart_shock");pushSkill(g,"heart",3,inRange.x,inRange.y,shock?185:120,.5,{grow:1.7});if(shock){const radius=g.meleeRange*1.8*(1+(shock-1)*.25);for(const m of g.mobs){const d=Math.hypot(m.x-inRange.x,m.y-inRange.y);if(d<radius){dealDamage(g,m,g.damage*.8*(1+(shock-1)*.15),"heart_shock");if(d>1){m.x+=(m.x-inRange.x)/d*18;m.y+=(m.y-inRange.y)/d*18}if(shock>=3)m.heartMark=3}}g.heartFx=.58;g.shake=9;burst(g,inRange.x,inRange.y,"#ff715b",24)}if(cardLevel(g,"fusion_heart_liver")&&inRange.poisonStacks){const blast=g.damage*.42*inRange.poisonStacks;for(const m of g.mobs)if(Math.hypot(m.x-inRange.x,m.y-inRange.y)<120)dealDamage(g,m,blast,"fusion_heart_liver");inRange.poisonStacks=0;burst(g,inRange.x,inRange.y,"#a8d43a",20)}if(cardLevel(g,"fusion_heart_brain"))shootAt(inRange,{damageMul:.65,chain:g.chainBonus,core:true,source:"fusion_heart_brain"})}
            }
            ...
            g.fire=g.fireRate*1.12/(1+speedBoost);const focus=cardLevel(g,"brain_focus"),toxic=cardLevel(g,"fusion_brain_liver");const targets=[...g.mobs].sort((a,b)=>toxic&&a.poisonStacks!==b.poisonStacks?b.poisonStacks-a.poisonStacks:focus?b.hp-a.hp:Math.hypot(a.x-g.x,a.y-g.y)-Math.hypot(b.x-g.x,b.y-g.y));for(let i=0;i<coreCount;i++){const target=targets[i%Math.max(1,targets.length)]||nearest;shootAt(target,{damageMul:focus&&(target.elite||target.boss)?.82*(1+levelValue(AUGMENT_BALANCE.brainFocus.eliteBossDamageBonus,focus)):.82,chain:cardLevel(g,"brain_chain")+g.chainBonus,poison:toxic?1:0,core:true,source:"brain_core",angle:Math.atan2(target.y-g.y,target.x-g.x)+(i-(coreCount-1)/2)*.035})}g.brainVolley=.24;pushSkill(g,"brain",1,g.x,g.y,74,.16,{rot:Math.atan2(nearest.y-g.y,nearest.x-g.x),grow:1.3});g.castFx=.16;g.castAngle=Math.atan2(nearest.y-g.y,nearest.x-g.x);sound.current?.play("shot");
            ...
            g.shots.push({x:g.x,y:g.y,vx:Math.cos(a)*690,vy:Math.sin(a)*690,life:.7,r:10+mo*2,damageMul:1+mo*.2,source:"lung_basic"});
            ...
            for(const m of g.mobs){const d=Math.hypot(m.x-g.x,m.y-g.y);if(d<range+m.r){dealDamage(g,m,g.damage*1.75,"muscle_basic");m.hit=.12;const nx=(m.x-g.x)/(d||1),ny=(m.y-g.y)/(d||1);m.kbX+=nx*kb;m.kbY+=ny*kb;hit=true}}
            ...
            g.fire=g.fireRate;const a=Math.atan2(nearest.y-g.y,nearest.x-g.x)+(gameRandom(g)-.5)*(g.organs.뇌<30?.34:0);for(let j=0;j<g.projectiles;j++)shootAt(nearest,{chain:g.chainBonus,angle:a+(j-(g.projectiles-1)/2)*.13});g.castFx=.16;g.castAngle=a;sound.current?.play("shot");
          }
        }
        for(const m of g.mobs){
          m.skill-=dt;m.hit-=dt;m.charge-=dt;m.toxin-=dt;m.poisonTick-=dt;m.heartMark-=dt;if(m.toxin>0||m.poisonStacks>0)dealDamage(g,m,(4+g.poison*1.5+m.poisonStacks*2.2)*dt,"poison_dot");
          ...
          if(m.elite&&m.skill<=0&&m.cast<=0&&m.charge<=0&&distanceToPlayer<680){
            m.skill=m.boss?3.8:4.6+gameRandom(g)*2.2;m.cast=m.boss ? 0.9 : 0.65;m.aimX=g.x;m.aimY=g.y;
          ...
          if(!m.boss&&!m.elite&&m.kind===1&&m.skill<=0&&m.cast<=0&&m.charge<=0&&distanceToPlayer<400){m.skill=3.2+Math.random()*2;m.cast=.45;m.aimX=g.x;m.aimY=g.y;g.warnings.push({x:m.x,y:m.y,tx:g.x,ty:g.y,life:.45,max:.45,kind:"line",r:24})}
          const weave=(!m.boss&&!m.elite&&m.kind===2)?Math.sin(g.t*5.5+(m.x+m.y)*.05)*.55:0;
          const a=m.charge>0?Math.atan2(m.aimY-m.y,m.aimX-m.x):Math.atan2(g.y-m.y,g.x-m.x)+weave;
          const move=m.charge>0?(m.elite?690:520):m.speed*(m.cast>0 ? 0.18 : 1);m.x+=Math.cos(a)*move*dt;m.y+=Math.sin(a)*move*dt;
          ...
          if(d<m.r+(coarse?12:16)&&g.inv<=0){hurtPlayer((m.boss?18:8)*diff.damage,m.x,m.y);if(m.boss&&g.bossWeakTarget){g.organs[g.bossWeakTarget]=Math.max(0,g.organs[g.bossWeakTarget]-4);g.effect=`노화 침식 · ${g.bossWeakTarget} -4`;g.effectT=1.2}g.inv=.55;g.shake=10;sound.current?.play("hurt");burst(g,g.x,g.y,"#ff715b",12)}
          if(g.poison&&d<95)dealDamage(g,m,g.poison*6*dt,"poison_aura");
          if((m.kbX||m.kbY)&&m.collideCd<=0){for(const o of g.mobs){if(o!==m&&o.hp>0&&Math.hypot(o.x-m.x,o.y-m.y)<m.r+o.r+4){const ex=g.damage*1.4;dealDamage(g,m,ex,"muscle_collision");dealDamage(g,o,ex,"muscle_collision");m.collideCd=.45;o.collideCd=Math.max(o.collideCd,.25);const mx=(m.x+o.x)/2,my=(m.y+o.y)/2;burst(g,mx,my,"#d8ff3e",8);pushSkill(g,"muscle",3,mx,my,64,.3,{grow:1.4});const ch=cardLevel(g,"muscle_chaincollide");if(ch){const rad=88*(1+(ch-1)*.3);for(const e of g.mobs)if(e!==m&&e!==o&&Math.hypot(e.x-mx,e.y-my)<rad)dealDamage(g,e,g.damage*.8,"muscle_chain_collision");g.shake=Math.max(g.shake,5)}break}}}
        }
        for(const s of g.shots){s.x+=s.vx*dt;s.y+=s.vy*dt;s.life-=dt;if(s.enemy){if(Math.hypot(s.x-g.x,s.y-g.y)<s.r+(coarse?11:15)&&g.inv<=0){s.life=0;hurtPlayer(7*diff.damage,s.x,s.y);if(g.bossWeakTarget&&g.mobs.some(m=>m.boss)){g.organs[g.bossWeakTarget]=Math.max(0,g.organs[g.bossWeakTarget]-2);g.effect=`노화 탄막 · ${g.bossWeakTarget} -2`;g.effectT=.9}g.inv=.42;g.shake=7;sound.current?.play("hurt");burst(g,g.x,g.y,"#ff715b",8)}}else{for(const m of g.mobs){if(Math.hypot(s.x-m.x,s.y-m.y)<s.r+m.r){const hit=g.damage*(s.r>9?1.65:1)*(s.damageMul??1)*g.rangedDamageMul;dealDamage(g,m,hit,s.source??"basic_projectile");s.life=0;m.hit=.08;if(s.poison){m.poisonStacks=Math.min(8,m.poisonStacks+s.poison);m.toxin=4}sound.current?.play("hit");burst(g,s.x,s.y,s.core?"#a49bd8":s.r>9?"#ff715b":"#d8ff3e",s.r>9?10:4);if((s.chain??0)>0){const next=g.mobs.filter(o=>o!==m&&o.hp>0&&Math.hypot(o.x-m.x,o.y-m.y)<220).sort((a,b)=>Math.hypot(a.x-m.x,a.y-m.y)-Math.hypot(b.x-m.x,b.y-m.y))[0];if(next){const a=Math.atan2(next.y-m.y,next.x-m.x);g.shots.push({x:m.x,y:m.y,vx:Math.cos(a)*610,vy:Math.sin(a)*610,life:.55,r:6,damageMul:(s.damageMul??1)*.7,chain:(s.chain??1)-1,poison:s.poison,core:true,source:"brain_chain"})}}break}}}}
        const concentrated=cardLevel(g,"liver_concentrated"),concentratedTick=concentrated?levelValue(AUGMENT_BALANCE.liverConcentrated.tickSeconds,concentrated):1;
        for(const f of g.fields){f.life-=dt;f.tick-=dt;for(const m of g.mobs)if(Math.hypot(m.x-f.x,m.y-f.y)<f.r){dealDamage(g,m,(7+f.stack*4)*dt,"liver_field");m.toxin=Math.max(m.toxin,.3);if(concentrated&&f.tick<=0){m.poisonStacks=Math.min(8,m.poisonStacks+1);m.toxin=4}}if(f.tick<=0)f.tick=concentrated?concentratedTick:1}g.fields=g.fields.filter(f=>f.life>0);
        if(cardLevel(g,"fusion_liver_brain")&&g.mainClass==="liver"&&g.toxicCoreCooldown<=0&&g.fields.length&&g.mobs.length){const source=g.fields[Math.floor(gameRandom(g)*g.fields.length)],target=[...g.mobs].sort((a,b)=>Math.hypot(a.x-source.x,a.y-source.y)-Math.hypot(b.x-source.x,b.y-source.y))[0];if(target){g.fields.push({x:target.x,y:target.y,r:48*g.poisonRadiusMul,life:3.2*g.poisonDurationMul,stack:1,kills:0,tick:0});g.toxicCoreCooldown=4.5;pushSkill(g,"liver",6,target.x,target.y,84,.45,{spin:5,grow:1.3});burst(g,target.x,target.y,"#a8d43a",14);g.effect="추적 독성 · 오염 지역 전파";g.effectT=.8}}
        const dead=g.mobs.filter(m=>m.hp<=0);for(const m of dead){
          g.kills++;burst(g,m.x,m.y,m.boss?"#ff715b":"#4ee5e1",m.boss?30:8);
          const closeKill=Math.hypot(m.x-g.x,m.y-g.y)<g.meleeRange*1.35,bloodflowLevel=cardLevel(g,"heart_bloodflow");if(closeKill&&(bloodflowLevel||cardLevel(g,"life_sports")))g.moveBuff=Math.max(g.moveBuff,bloodflowLevel?levelValue(AUGMENT_BALANCE.heartBloodflow.durationSeconds,bloodflowLevel):2);
          const circulationLevel=cardLevel(g,"lung_circulation");if(g.mainClass==="lung"&&circulationLevel){const duration=levelValue(AUGMENT_BALANCE.lungCirculation.durationSeconds,circulationLevel);g.galeKillLock=duration;g.moveBuff=Math.max(g.moveBuff,duration)}
          if(cardLevel(g,"common_regen")){g.killsSinceRegen++;if(g.killsSinceRegen>=20){g.killsSinceRegen=0;const heal=g.maxHp*.08*(1-g.recoveryPenalty);healPlayer(g,heal);g.effect=`재생 인자 · 체력 +${Math.round(heal)}`;g.effectT=1.1}}
          if(cardLevel(g,"liver_rupture")&&m.poisonStacks>0){const blast=(7+g.poison*2)*m.poisonStacks*1.5*(1+(cardLevel(g,"liver_rupture")-1)*.2);for(const other of g.mobs)if(other!==m&&Math.hypot(other.x-m.x,other.y-m.y)<145){dealDamage(g,other,blast,"liver_rupture");other.poisonStacks=Math.min(8,other.poisonStacks+1);other.toxin=4}pushSkill(g,"liver",5,m.x,m.y,150,.5,{grow:1.7});burst(g,m.x,m.y,"#a8d43a",22);g.effect="독성 파열 · 연쇄 오염";g.effectT=.8}
          ...
          const deathField=g.fields.find(f=>Math.hypot(m.x-f.x,m.y-f.y)<f.r);if(deathField&&cardLevel(g,"fusion_liver_heart")){deathField.kills++;if(deathField.kills>=3){deathField.kills=0;for(const other of g.mobs)if(Math.hypot(other.x-deathField.x,other.y-deathField.y)<deathField.r*1.5)dealDamage(g,other,g.damage*2.1,"fusion_liver_heart");burst(g,deathField.x,deathField.y,"#ff715b",28);g.shake=10;g.effect="독성 폭주 · 지대 폭발";g.effectT=1}}
          const frenzy=cardLevel(g,"brain_frenzy");if(g.mainClass==="brain"&&frenzy&&g.kills%levelValue(AUGMENT_BALANCE.brainFrenzy.killsPerProc,frenzy)===0){const coreCount=2+cardLevel(g,"brain_synapse"),targets=[...g.mobs].filter(o=>o!==m&&o.hp>0).sort((a,b)=>Math.hypot(a.x-g.x,a.y-g.y)-Math.hypot(b.x-g.x,b.y-g.y));for(let i=0;i<coreCount&&targets.length;i++){const target=targets[i%targets.length],a=Math.atan2(target.y-g.y,target.x-g.x);g.shots.push({x:g.x,y:g.y,vx:Math.cos(a)*650,vy:Math.sin(a)*650,life:1.4,r:8,damageMul:1.15,chain:cardLevel(g,"brain_chain")+g.chainBonus,core:true,source:"brain_frenzy"})}g.brainVolley=.5;pushSkill(g,"brain",5,g.x,g.y,175,.5,{grow:1.7});g.effect="사고 폭주 · 코어 일제 사격";g.effectT=1}
          const dropCount=m.boss?7:1;
          for(let i=0;i<dropCount;i++){
            const roll=gameRandom(g),a=gameRandom(g)*6.28,s=35+gameRandom(g)*90;
            const kind=m.boss?(i<3?"xp":i<5?"heal":"organ"):(roll<.72?"xp":roll<.88?"heal":"organ");
            g.drops.push({x:m.x,y:m.y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,kind,organ:kind==="organ"?ORGAN_KEYS[Math.floor(gameRandom(g)*ORGAN_KEYS.length)]:undefined,value:kind==="xp"?(m.boss?4:1):kind==="heal"?(m.boss?18:7):(m.boss?5:2),life:14,phase:Math.random()*6.28});
          }
          if(m.boss){g.telemetry.bossResults.push({stage:g.stage+1,killTime:Number(g.t.toFixed(2)),playerLevel:g.level,hpPercent:Math.max(0,Math.round(g.hp/g.maxHp*100))});if(g.stage===3&&g.stageT>BOSS_AT){endGame(true)}else if(!g.augmentDone){g.augmentDone=true;openChoice("전투 증강",weightedChoices(g,"boss"))}}
        }
        ...
        for(const d of g.drops){
          ...
          if(dist<23){
            ...
            }else if(d.kind==="heal"){const healed=Math.max(1,Math.round(d.value*(1-g.recoveryPenalty)));healPlayer(g,healed);picked=`회복 세포 +${healed}`}
            else if(d.organ){g.organs[d.organ]=Math.min(100,g.organs[d.organ]+d.value);if(d.organ==="뇌")g.fireRate=Math.max(.18,g.fireRate*.998);else if(d.organ==="심장"){g.maxHp+=.15;healPlayer(g,.15)}else if(d.organ==="폐")g.speed+=.35;else if(d.organ==="간")g.poison+=.025;else{g.damage+=.1;g.armor+=.025}picked=`${d.organ} 특화 영양소 +${d.value}`}
    ...
    const loop=(now:number)=>{
      const g=game.current,frameDt=Math.min(.033,(now-(g.last||now))/1000);g.last=now;
      ...
      if(!g.paused&&mode==="play"){
        ...
        else{g.simulationAccumulator+=frameDt;stepCount=Math.floor(g.simulationAccumulator/SIMULATION_DT);g.simulationAccumulator-=stepCount*SIMULATION_DT}
      ...
      for(const d of g.drops){
        ...
        const bob=Math.sin(d.phase)*3;ctx.save();ctx.translate(d.x,d.y+bob);
        const dropGlow=d.kind==="xp"?"#d8ff3e":d.kind==="heal"?"#ff715b":"#4ee5e1";
        // 배경 대비용 어두운 받침 + 광도 강화
        ctx.fillStyle="rgba(0,0,0,.34)";ctx.beginPath();ctx.ellipse(0,3,15*renderScale,6*renderScale,0,0,6.28);ctx.fill();
        ctx.rotate(d.phase*.35);ctx.shadowBlur=22;ctx.shadowColor=dropGlow;
        if(itemArt.complete&&itemArt.naturalWidth){const row=d.kind==="xp"?0:d.kind==="heal"?1:2,size=46*renderScale;ctx.drawImage(itemArt,g.stage*384,row*(1024/3),384,1024/3,-size/2,-size/2,size,size)}
        else{ctx.fillStyle=dropGlow;ctx.beginPath();ctx.arc(0,0,10,0,6.28);ctx.fill()}
        ctx.restore();
      }
      for(const s of g.shots){
        ...
        ctx.save();ctx.translate(s.x,s.y);ctx.shadowBlur=s.enemy?16:18;ctx.shadowColor=glow;
        // 적 탄환은 어두운 외곽으로 배경과 분리해 "날아오는 게 보이도록"
        if(s.enemy){ctx.fillStyle="rgba(10,4,4,.5)";ctx.beginPath();ctx.arc(0,0,s.r+4,0,6.28);ctx.fill()}
        if(projectileArt.complete&&projectileArt.naturalWidth){
          const cellW=projectileArt.naturalWidth/4,cellH=projectileArt.naturalHeight/2,col=sprite%4,row=Math.floor(sprite/4),size=(s.enemy?44:s.r>9?56:38)*renderScale;
          ctx.drawImage(projectileArt,col*cellW,row*cellH,cellW,cellH,-size/2,-size/2,size,size);
        }else{ctx.fillStyle=glow;ctx.beginPath();ctx.arc(0,0,(s.r+2)*renderScale,0,6.28);ctx.fill()}
        ctx.restore();
      }ctx.shadowBlur=0;
      for(const m of g.mobs){
        ...
        if(atlas.complete&&atlas.naturalWidth){ctx.shadowColor="rgba(0,0,0,.6)";ctx.shadowBlur=(m.boss?12:8)*renderScale;ctx.drawImage(atlas,frame*cell,idx*cell,cell,cell,-size/2,-size*.58,size,size);ctx.shadowBlur=0}
      ...
      ctx.globalAlpha=g.inv>0&&Math.floor(g.t*20)%2 ? .38 : 1;
      if(playerArt.complete&&playerArt.naturalWidth){const sx=(formIndex%4)*384,sy=Math.floor(formIndex/4)*512,dx=-playerSize/2,dy=-playerSize*.64;
        // 1패스: 어두운 외곽 대비 → 2패스: 애시드 글로우
        ctx.shadowColor="rgba(0,0,0,.55)";ctx.shadowBlur=10*renderScale;ctx.drawImage(playerArt,sx,sy,384,512,dx,dy,playerSize,playerSize);
        ctx.shadowColor="#d8ff3e";ctx.shadowBlur=20;ctx.drawImage(playerArt,sx,sy,384,512,dx,dy,playerSize,playerSize);}
      else{ctx.shadowBlur=22;ctx.shadowColor="#d8ff3e";ctx.fillStyle="#d8ff3e";ctx.beginPath();ctx.arc(0,0,18,0,6.28);ctx.fill()}
      ctx.restore();ctx.globalAlpha=1;ctx.shadowBlur=0;
      ctx.restore();
      // 피격 방향 비네트: 맞은 쪽으로 붉은 빛이 쏠린다 (rAF로 부드럽게)
      if(hurtRef.current){hurtRef.current.style.opacity=String(Math.min(.92,g.hurtT*1.8));hurtRef.current.style.setProperty("--hx",(50+Math.cos(g.hurtDir)*48)+"%");hurtRef.current.style.setProperty("--hy",(50+Math.sin(g.hurtDir)*48)+"%")}
      if(lowRef.current)lowRef.current.style.opacity=(!g.invuln&&g.hp>0&&g.hp/g.maxHp<.3&&mode==="play")?"1":"0";
  ...
  const choose=(c:Choice)=>{const g=game.current;c.apply(g);g.telemetry.choices.push({id:c.id??null,name:c.name,kind:c.kind??null,tier:c.tier??null,choiceType,time:Number(g.t.toFixed(2)),stage:g.stage+1,playerLevel:g.level,cardLevel:c.id&&c.maxLevel?cardLevel(g,c.id):null});g.effect=`${c.name} · ${c.effect}`;g.effectT=2.4;if(c.kind==="fusion"&&c.id){const set=new Set<string>(JSON.parse(localStorage.getItem("organ-fusions")||"[]"));set.add(c.id);const arr=[...set];localStorage.setItem("organ-fusions",JSON.stringify(arr));setFoundFusions(arr)}if(c.chemistry&&!g.chemistries.includes(c.chemistry)){g.chemistries.push(c.chemistry);g.effect=`케미 발견 · ${c.name}`;g.effectT=2.8;const found=new Set<string>(JSON.parse(localStorage.getItem("organ-chemistry")||"[]"));found.add(c.chemistry);const chemistries=[...found];localStorage.setItem("organ-chemistry",JSON.stringify(chemistries));setArchive(old=>({...old,chemistries}))}ORGAN_KEYS.forEach(k=>g.organs[k]=Math.max(0,Math.min(100,g.organs[k])));if(choiceType==="생활 선택")g.choices.push(c.name);else g.augments.push(c.name);sendGameLabEvent("game_choice_selected",{runNumber:runNumber.current,choiceType,choice:c.name,effect:c.effect,elapsedSeconds:Math.round(g.t),stage:g.stage+1,level:g.level,organLevels:{...g.organLevels},mainClass:g.mainClass});
  ...
  return <main className="game-shell"><section className="frame">
    ...
    {(mode==="play"||mode==="pause")&&<><div ref={hurtRef} className="hurt-vignette" aria-hidden="true"/><div ref={lowRef} className="low-hp-vignette" aria-hidden="true"><b>체력 위험</b></div><div className="hud"><div className="hud-top"><div className="stage"><small>LIFE STAGE 0{hud.stage+1}</small>{STAGES[hud.stage][0]}<span className="build-chip">주력 {leaders.map((k,i)=><span key={k}>{i>0?" + ":""}<OrganGlyph k={k} size={11}/> {k}</span>)}</span></div><div><div className="clock">{fmt(hud.t)} <small>/ 6:00</small></div><div className={`hp ${hud.hp/hud.max<.3?"low":""}`}><i style={{width:`${Math.max(0,hud.hp/hud.max*100)}%`}}/><b>{Math.max(0,Math.ceil(hud.hp))} / {Math.ceil(hud.max)}</b></div></div></div></div>
      <aside className={`chemistry-panel ${activeClass?"awakened":""}`}><small>MAIN CLASS</small>{activeClass?<><div className="chemistry-icons"><span><OrganGlyph k={activeClass.key} size={22}/></span></div><h3>{activeClass.className}</h3><p>{activeClass.key} Lv.3 각성 · SPACE {activeClass.action}</p><em>이번 생애의 주 직업으로 고정</em></>:<><h3>각성 탐색 중</h3><p>심장·뇌·간·폐·근육 중 하나를 Lv.3까지 성장시키세요.</p></>}</aside>
      {/* 상단 알림은 한 번에 하나만: 토스트(effect)가 뜨면 목표 표시는 감춘다. 카운트다운 멘트 대신 얇은 진행 바로 대체 */}
      {hud.effect?<div className="organ-effect">{hud.effect}</div>:(()=>{const preFirst=hud.t<FIRST_CHOICE_AT;const bossUp=game.current.stageT>=BOSS_AT;const label=preFirst?"첫 생활 선택 준비 중":bossUp?`${STAGES[hud.stage][1]} 처치`:`다음 · ${STAGES[hud.stage][1]}`;const prog=bossUp?1:preFirst?hud.t/FIRST_CHOICE_AT:Math.min(1,game.current.stageT/BOSS_AT);return <div className="next-objective"><b>{label}</b>{!bossUp&&<i><u style={{width:`${Math.round(prog*100)}%`}}/></i>}</div>})()}
    ...
    {mode==="choice"&&<div className={`choice-wrap choice-${choiceType==="세포 진화"?"evolve":choiceType==="빌드 각성"||choiceType==="장기 각성"?"build":"life"}`}><div className="choice-head"><div><div className="eyebrow">{choiceType==="생활 선택"?"LIFE INTERRUPT":choiceType==="장기 각성"?"ORGAN AWAKENING":choiceType==="빌드 각성"?"BUILD AWAKENING":"CELL EVOLUTION"}</div><h2>{choiceType==="세포 진화"?"장기 성장":choiceType}</h2>{choiceType==="생활 선택"&&!hud.mainClass&&<small className="choice-sub">선택한 생활이 장기 성향을 정합니다 · 장기를 Lv.3까지 키우면 그 직업으로 각성합니다</small>}</div><p><b>1 · 2 · 3</b> 즉시 선택&nbsp;&nbsp; <b>A / D</b> 이동&nbsp;&nbsp; <b>SPACE</b> 확정</p></div><div className={`cards ${cards.length===2?"two":""}`}>{cards.map((c,i)=>{const tags=cardOrgans(c),nextLevel=c.id?cardLevel(game.current,c.id)+1:0;const accent=c.kind==="fusion"?"#d8ff3e":c.kind==="common"?"#4ee5e1":tags.length?ORGAN_META[tags[0]].color:c.kind==="life"?"#e0c24e":"#d8ff3e";return <button style={{"--card-accent":accent} as React.CSSProperties} className={`card card-${c.kind||"general"} ${c.tier?`augment-tier-${c.tier}`:""} ${selectedCard===i?"selected":""} ${c.awakening&&c.awakening!=="hold"?"awakening-card":""}`} key={c.name} onMouseEnter={()=>setSelectedCard(i)} onFocus={()=>setSelectedCard(i)} onClick={()=>choose(c)} aria-selected={selectedCard===i}><span className="card-no"><kbd>{i+1}</kbd> 선택 {c.kind==="fusion"&&<em>FUSION</em>}</span><div className="card-art">{tags.length?tags.map(k=><span key={k} style={{"--organ-color":ORGAN_META[k].color,color:ORGAN_META[k].color} as React.CSSProperties}><OrganGlyph k={k} size={30}/></span>):<span>✦</span>}</div><div className="organ-tags">{c.tier&&<span className="card-tier">T{c.tier}</span>}{tags.map(k=><span key={k}><OrganGlyph k={k} size={12}/> {k}{c.organLevel&&` Lv.${game.current.organLevels[c.organLevel]} → Lv.${Math.min(3,game.current.organLevels[c.organLevel]+1)}`}</span>)}{c.maxLevel===3&&<span>Lv.{nextLevel}</span>}</div><h3>{c.name}</h3><p>{c.desc}</p><div className="decision-effects"><strong><small>플레이 변화</small>{c.effect}</strong>{c.cost&&<em><small>대가</small>{c.cost}</em>}</div>{selectedCard===i&&<small className="confirm-hint">SPACE로 확정</small>}</button>})}</div></div>}
    ...
    {mode==="report"&&<div className="screen report"><div className="report-grid"><div><div className="eyebrow">LIFE REPORT / COMPLETE</div><h1>{report.win?"노화를 넘어섰습니다.":"생애가 끝났습니다."}</h1><p className="report-copy">{mainName?<><b>{mainName}</b>{josa(mainName,"으로","로")} 살아낸 생애였습니다. {report.fusions.length?<>융합 <b>{report.fusions.join(", ")}</b>{josa(report.fusions[report.fusions.length-1],"을","를")} 손에 넣었습니다. </>:null}다음 생애에는 <b>타고난 {strongest}</b>{josa(strongest,"이","가")} 유전됩니다.</>:<>주 직업을 각성하지 못한 채 스러진 생애였습니다. 다음 생애에는 <b>타고난 {strongest}</b>{josa(strongest,"이","가")} 유전됩니다.</>}</p><div className="stats"><div className="stat"><small>SURVIVAL</small><b>{fmt(report.t)}</b></div><div className="stat"><small>ZOMBIES</small><b>{report.kills} 처치</b></div><div className="stat"><small>BUILD</small><b>{build}</b></div><div className="stat"><small>GENE</small><b>타고난 {strongest}</b></div></div>{report.telemetry&&<div className="telemetry-summary"><div><small>DAMAGE</small><b>{Math.round(report.telemetry.damageDealt).toLocaleString()}</b></div><div><small>TAKEN / HITS</small><b>{Math.round(report.telemetry.damageTaken)} / {report.telemetry.hitsTaken}</b></div><div><small>MOVE / ACTION</small><b>{Math.round(report.telemetry.distanceTraveled)} / {report.telemetry.actionsUsed}</b></div><div><small>BOSSES</small><b>{report.telemetry.bossKills}</b></div></div>}<div className="report-actions"><button className="primary" onClick={()=>start(game.current.difficulty)}>같은 난이도로 다시 ↗</button><button className="report-menu-btn" onClick={()=>{game.current.paused=true;sound.current?.stopMusic();setMenuSection("home");setMode("start")}}>메인 화면으로</button></div></div><div><div className="organ-report">{ORGAN_KEYS.map(k=><div className="organ-line" key={k}><span>{k}</span><div className="bar"><i style={{width:`${report.organs[k]}%`}}/></div><b>{report.organs[k]}</b></div>)}</div><p className="gene">생활: {report.choices.join(" · ")||"기록 없음"}<br/>증강: {report.augments.join(" · ")||"기록 없음"}</p>{report.telemetry&&<div className="telemetry-export"><div><small>RUN TELEMETRY</small><b>{report.telemetry.debug?"DEBUG RUN":report.telemetry.benchmark?`BENCHMARK ${benchmarkBatch.length}/${benchmarkRunTarget}`:"VALID RUN"} · 최근 {telemetryRuns.length}회 저장</b></div><button onClick={()=>downloadTelemetry(`organ-run-${report.telemetry?.runId}.json`,report.telemetry)}>현재 JSON</button><button onClick={()=>downloadTelemetry("organ-run-history.json",{schemaVersion:1,exportedAt:new Date().toISOString(),runs:telemetryRuns})}>누적 JSON</button>{report.telemetry.benchmark&&benchmarkBatch.length>=benchmarkRunTarget&&<button onClick={()=>downloadTelemetry(`organ-benchmark-${report.telemetry?.benchmarkTarget}.json`,{schemaVersion:1,exportedAt:new Date().toISOString(),runs:benchmarkBatch})}>배치 JSON</button>}<details><summary>JSON 미리보기</summary><pre>{JSON.stringify(report.telemetry,null,2)}</pre></details>{report.telemetry.benchmark&&benchmarkBatch.length>=benchmarkRunTarget&&<details><summary>배치 JSON 미리보기</summary><pre data-testid="benchmark-batch-json">{JSON.stringify({schemaVersion:1,exportedAt:new Date().toISOString(),runs:benchmarkBatch},null,2)}</pre></details>}</div>}</div></div></div>}
  </section></main>;
}
