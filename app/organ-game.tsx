...
type SoundName = "start"|"dash"|"shot"|"hit"|"hurt"|"pickup"|"level"|"boss"|"heart"|"win"|"lose";
...
function createSoundEngine(){
  ...
  const play=(name:SoundName)=>{
    ...
    gain.gain.setValueAtTime((name==="boss"||name==="hurt") ? 0.1 : 0.055,audio.currentTime);gain.gain.exponentialRampToValueAtTime(.0001,audio.currentTime+duration);
...
function fresh(difficulty:Difficulty="normal"):Game {
  return {w:1280,h:720,worldW:2400,worldH:1600,t:0,stage:0,stageT:0,hp:100,maxHp:100,x:1200,y:800,vx:0,vy:0,dash:0,dashCharges:2,maxDash:2,inv:0,fire:0,kills:0,
...
export default function OrganGame() {
  ...
  const [hud,setHud]=useState({hp:100,max:100,t:0,stage:0,organs:game.current.organs,level:1,xp:0,nextXp:12,loot:"",effect:"",chemistries:[] as string[],dashCharges:2,maxDash:2});
  ...
  useEffect(()=>{
    ...
    const drawEnvironment=(g:Game)=>{
      ...
      if(map.complete&&map.naturalWidth)ctx.drawImage(map,0,0,g.worldW,g.worldH);
      ctx.strokeStyle="rgba(216,255,62,.32)";ctx.lineWidth=4;ctx.strokeRect(2,2,g.worldW-4,g.worldH-4);
    };
  ...
  return <main className="game-shell"><section className="frame">
    ...
    {(mode==="play"||mode==="pause")&&<><div className="hud"><div className="hud-top"><div className="stage"><small>LIFE STAGE 0{hud.stage+1}</small>{STAGES[hud.stage][0]}<span className="build-chip">주력 {leaders.map(k=>`${ORGAN_META[k].icon} ${k}`).join(" + ")}</span></div><div><div className="clock">{fmt(hud.t)} <small>/ 8:00</small></div><div className="hp"><i style={{width:`${Math.max(0,hud.hp/hud.max*100)}%`}}/></div></div></div></div>
      ...
      <div className="dash-hint"><span>SPACE 2단 대시</span><i>{Array.from({length:hud.maxDash},(_,i)=><b className={i<hud.dashCharges?"ready":""} key={i}/>)}</i><em>{hud.dashCharges?`${hud.dashCharges}회 준비`:"재충전 중"}</em></div></>}
