...
import { createAugmentCatalog, getAugmentBranch, getAvailableAugments, pickTieredAugment, recordAugmentPick } from "../app/game/augment-selection.js";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const balance = JSON.parse(await readFile(path.join(root, "app/game/augment-balance.json"), "utf8"));
const augmentEntries = JSON.parse(await readFile(path.join(root, "app/game/augment-catalog.json"), "utf8"));
const augmentCatalog = createAugmentCatalog(augmentEntries);
const augmentCards = augmentEntries.map(entry => ({ ...entry, maxLevel: 3 }));

const CLASS_KEYS = ["heart", "brain", "liver", "lung", "muscle"];
const CLASS_NAMES = { heart:"심장", brain:"뇌", liver:"간", lung:"폐", muscle:"근육" };
const CLASS_CARDS = Object.fromEntries(CLASS_KEYS.map(key => [key, augmentEntries.filter(entry => entry.main === key).map(entry => entry.id)]));
...
function makeAugmentOffer(state,random,source){
  const picks=[],excluded=new Set();
  const availableBefore=getAvailableAugments({state,cards:augmentCards,catalog:augmentCatalog});
  if(!availableBefore.length)return picks;
  const pityDue=state.tierPity>=balance.tierSystem.pityAfterOffers-1;
  const weakestTier=pityDue?balance.tierSystem.pityWeakestGuaranteedTier:source==="boss"?balance.tierSystem.bossWeakestGuaranteedTier:0;
  if(weakestTier){
    const guaranteed=pickTieredAugment({state,cards:augmentCards,catalog:augmentCatalog,config:balance,excluded,weakestTier,random});
    if(guaranteed){picks.push(guaranteed);excluded.add(guaranteed.id)}
  }
  while(picks.length<3){
    const available=getAvailableAugments({state,cards:augmentCards,catalog:augmentCatalog,excluded});
    if(available.length&&random()<.45){
      const card=pickTieredAugment({state,cards:augmentCards,catalog:augmentCatalog,config:balance,excluded,random});
      if(card){picks.push(card);excluded.add(card.id);continue}
    }
    picks.push({id:`other_${state.offers}_${picks.length}`,tier:0});
  }
  const classPicks=picks.filter(card=>card.tier);
  state.tierPity=classPicks.some(card=>card.tier<=balance.tierSystem.pityWeakestGuaranteedTier)?0:state.tierPity+1;
  state.offers++;
...
function chooseAugment(state,cards,random){
  const classCards=cards.filter(card=>card.tier);
  if(!classCards.length)return;
  const scored=classCards.map(card=>{
    const owned=(state.cardLevels[card.id]||0)>0?4:0;
    const branch=getAugmentBranch(augmentCatalog,card.id);
    const sameBranch=state.acquiredCards.some(id=>getAugmentBranch(augmentCatalog,id)===branch)?2:0;
    return{card,score:owned+sameBranch+(5-card.tier)*.2+random()*.01};
  });
  return scored.sort((a,b)=>b.score-a.score)[0].card;
}

function makeChoice(state,random,forced,source="level"){
  if(!state.awakened){
    state.organGrowth++;if(state.organGrowth>=3){state.awakened=true;state.mainClass=state.classKey;state.awakeningAt=state.t;if(forced){for(const id of CLASS_CARDS[state.classKey]){state.cardLevels[id]=1;state.acquiredCards.push(id)}state.cardLevels[forced.id]=forced.level}}
    return;
  }
  if(forced)return;
  const offer=makeAugmentOffer(state,random,source),chosen=chooseAugment(state,offer,random);if(!chosen)return;
  const firstPick=!state.acquiredCards.includes(chosen.id);
  state.cardLevels[chosen.id]=(state.cardLevels[chosen.id]||0)+1;
  if(firstPick)state.acquiredCards.push(chosen.id);
  recordAugmentPick(state,augmentCatalog,chosen.id);
  state.classCardsChosen++;state.tierCardsChosen[chosen.tier]++;
  if(chosen.tier===1&&state.firstTier1At===null){state.firstTier1At=state.t;state.firstTier1Level=state.level}
  const branch=getAugmentBranch(augmentCatalog,chosen.id),branchEntries=augmentEntries.filter(entry=>entry.main===state.classKey&&getAugmentBranch(augmentCatalog,entry.id)===branch);
  if(state.firstBranchCompleteAt===null&&branchEntries.every(entry=>state.acquiredCards.includes(entry.id)))state.firstBranchCompleteAt=state.t;
}
...
function simulateRun({classKey,seed,forced}){
  ...
  for(;state.t<480&&state.hp>0&&!win;state.t+=dt){
    ...
    for(const boss of defeatedBosses){state.bossesKilled++;state.choices++;makeChoice(state,random,forced,"boss");if(boss.stage===3)win=true}
