...
const rows=classes.map(key=>{const item=byClass[key];return`| ${names[key]} | ${item.runs} | ${item.awakeningRate}% | ${item.clearRate}% | ${item.avgSurvivalSeconds}s | ${item.avgKills} | ${item.avgDps} | ${item.avgDamageTakenPerMinute} | ${item.avgHitsPerMinute} | ${item.avgBossKills} | ${item.avgActionsPerMinute} |`}).join("\n");
